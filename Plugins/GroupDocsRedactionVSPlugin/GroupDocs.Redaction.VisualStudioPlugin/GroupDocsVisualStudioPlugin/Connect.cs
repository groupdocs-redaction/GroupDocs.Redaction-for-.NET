...
using GroupDocsRedactionVisualStudioPlugin.Properties;

namespace GroupDocsRedactionVisualStudioPlugin
{
    public class Connect : Extensibility.IDTExtensibility2, IDTCommandTarget
    {
        ...
        private const string GROUPDOCS_MENU_TOOLTIP = "Download and Run GroupDocs.Redaction for .NET API Examples";

        private const string Redaction_COMMAND_NAME = "GroupDocsRedactionVSPlugin";
        private const string Redaction_COMMAND_CAPTION = "New GroupDocs.Redaction Example Project";
        private const string Redaction_COMMAND_TOOLTIP = "Download and Run GroupDocs.Redaction for .NET API Examples";
            ...
            try
            {
                ...
                for (int iloop = 1; iloop <= toolsPopup.CommandBar.Controls.Count; iloop++)
                {
                    if (toolsPopup.CommandBar.Controls[iloop].Caption == GROUPDOCS_MENU_CAPTION)
                    {
                        ...
                        {
                            if (cmdbtn.Caption == Redaction_COMMAND_CAPTION)
                            {
                                oBtn = cmdbtn;
                            }
                        }
                ...
                if (oBtn == null)
                {
                    ...
                    Command myCommand = commands.AddNamedCommand2(addInInstance, Redaction_COMMAND_NAME,
                        Redaction_COMMAND_CAPTION, Redaction_COMMAND_TOOLTIP, false, Resources.pnglogosmall, ref contextGUIDS,
           ...
           ref object varOut, ref bool handled)
        {
            try
            {
                ...
                {
                    if (cmdName == addInInstance.ProgID + "." + Redaction_COMMAND_NAME)
                    {
