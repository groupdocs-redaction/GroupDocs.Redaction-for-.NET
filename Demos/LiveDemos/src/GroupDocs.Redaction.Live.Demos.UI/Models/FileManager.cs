...
namespace GroupDocs.Redaction.Live.Demos.UI.Models
{
	public static class FileManager
	{
		public static FileUploadResult UploadFile(string file, string userEmail)
		{
			...
			try
			{
				...
				System.Threading.Tasks.Task taskUpload = httpClient.PostAsync(GroupDocs.Redaction.Live.Demos.UI.Config.Configuration.GroupDocsAppsAPIBasePath + "api/GroupDocsAppsweb/UploadFile" + "?userEmail=" + userEmail, content).ContinueWith(task =>
				{
		...
		public static FileUploadResult UploadFileToFolder(string file, string userEmail, string folder)
		{
			...
			try
			{
				...
				System.Threading.Tasks.Task taskUpload = httpClient.PostAsync(GroupDocs.Redaction.Live.Demos.UI.Config.Configuration.GroupDocsAppsAPIBasePath + "api/GroupDocsAppsweb/UploadFileToFolder" + "?userEmail=" + userEmail + "&folder=" + folder, content).ContinueWith(task =>
				{
					if (task.Status == System.Threading.Tasks.TaskStatus.RanToCompletion)
					{
						var response = task.Result;
		...
		public static FileUploadResult UploadFile(Stream fileStream, string fileName, string userEmail)
		{
			...
			try
			{
				...
				try
				{
					taskUpload.Wait((GroupDocs.Redaction.Live.Demos.UI.Config.Configuration.ThreadAbortSeconds + 4) * 1000, new CancellationToken(false));
				}
				catch (Exception exc)
				{
					httpClient.Dispose();
		...
		public static HttpResponseMessage DownloadFile(string fileName)
		{
			...
			{
				var url = GroupDocs.Redaction.Live.Demos.UI.Config.Configuration.GroupDocsAppsAPIBasePath + "api/GroupDocsAppsweb/DownloadFile?outFile=" + fileName;
